...
from numpy import empty, sinh, cosh, reshape
...
def verify1d(array: f32[:], result: f32[:], size: i32):
    ...
    for i in range(size):
        assert abs( sinh (sinh(array[i])%20) - result[i]) <= eps
...
def elemental_sinh():
    ...
    for i in range(40):
        for j in range(10):
            for k in range(16):
                for l in range(2):
                    arraynd[i, j, k, l] = float(i + 2*j + 3*k + 4*k)

    sinhnd = ((sinh(arraynd)%3) +2)/2
...
def elemental_trig_identity():
    ...
    arraynd: f64[10, 5, 2, 4] = empty((10, 5, 2, 4))

    identity1: f64[10, 5, 2, 4] = empty((10, 5, 2, 4))
    identity2: f64[10, 5, 2, 4] = empty((10, 5, 2, 4))
    identity3: f64[10, 5, 2, 4] = empty((10, 5, 2, 4))
    identity4: f64[10, 5, 2, 4] = empty((10, 5, 2, 4))

    observed1d_1: f64[400] = empty(400)
    observed1d_2: f64[400] = empty(400)
    observed1d_3: f64[400] = empty(400)
    observed1d_4: f64[400] = empty(400)
    ...
    identity1 =  1.0 - cosh(arraynd/4)**2 + sinh(arraynd/4)**2
    identity2 =  cosh(-1 * arraynd) - cosh(arraynd)
    identity3 =  sinh(-1 * arraynd) + sinh(arraynd)
    identity4 =  cosh(arraynd/4 + arraynd/2) - cosh(arraynd/4)* cosh(arraynd/2) - sinh(arraynd/4)* sinh(arraynd/2)
